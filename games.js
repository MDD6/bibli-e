(function(){
  ...
  ].map(j => ({ ...j, disponiveis: Math.floor(Math.random()*j.total) }));
  ...
  function render(){
    ...
    const filtrado = jogos.filter(j => (!q || j.titulo.toLowerCase().includes(q)) && (!cat || j.categoria===cat));
    grid.innerHTML = filtrado.map(j => {
      ...
      const reservarBtn = j.disponiveis===0 ? `<button class='reserve-btn' data-game='${j.id}'>Reservar</button>` : `<button class='reserve-btn' disabled>Disponível</button>`;
